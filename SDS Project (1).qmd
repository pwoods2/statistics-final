...
In our initial analysis of the dataset, we had 1,236 observations (rows), but discovered some missing data for key variables, particularly in parental height and smoking status. To address this, we removed them from the set, ensuring our results would not be biased and leaving us with a remainder of 727 observations. 

#### **Interest**

This dataset is particularly interesting to me because it relates to my major. I am a Human Development and Family Sciences Major, which allows us to explore the relationships between various maternal and paternal factors and the gestation period. Understanding these relationships can provide insights into factors that influence pregnancy outcomes, which is valuable for public health and medical research.
...
-   We chose to use the all-subsets model selection. This technique identified that a spline transformation of log(wt) and the variable parity were the most important predictors for the outcome log(gestation). Optimizing the model based on the AIC criterion, predictors were added to the model based on their statistical significance and contributions to reducing the AIC, and removed if they were indicated to not improving the model. The AIC for the model log(wt) and parity was -4406.08, the lowest value, making it the most effective in explaining gestation length. 
